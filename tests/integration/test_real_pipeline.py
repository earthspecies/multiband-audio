...
from multiband_audio import LinearHead, MultibandTransform, MultibandWrapper, build_fusion
...
def make_wrapper(fusion_name: str, **fusion_kwargs) -> MultibandWrapper:
...
class TestRealAudioLoading:
    ...
    def test_expected_num_bands(self):
    ...
    def test_band_values_differ(self):
...
class TestRealForwardPass:
    def test_full_pipeline_gp(self):
    ...
    def test_full_pipeline_moe(self):
    ...
    def test_full_pipeline_sa(self):
    ...
    def test_full_pipeline_mp(self):
    ...
    def test_full_pipeline_hybrid_with_real_scores(self):
        ...
        assert scores.shape == (2, bands.shape[1], 2), \
            f"Expected scores (N, B, 2), got {scores.shape}"
    ...
    def test_hybrid_scores_actually_used(self):
...
class TestTrainingLoop:
    ...
    def test_gradient_flows_to_backbone(self):
        ...
        wrapper = MultibandWrapper(backbone=backbone, fusion="mp", head=head,
                                   embed_dim=EMBED_DIM, freeze_backbone=False)
    ...
    def test_frozen_backbone_no_grad(self):
        ...
        head = LinearHead(EMBED_DIM, NUM_CLASSES)
        wrapper = MultibandWrapper(backbone=backbone, fusion="mp", head=head,
                                   embed_dim=EMBED_DIM, freeze_backbone=True)
    ...
    def test_hybrid_backward_pass(self):
        ...
        backbone = SmallConvBackbone(embed_dim=EMBED_DIM)
        head = LinearHead(EMBED_DIM, NUM_CLASSES)
        wrapper = MultibandWrapper(backbone=backbone, fusion="hyb", head=head,
                                   embed_dim=EMBED_DIM, num_handcrafted=2)
        optimizer = torch.optim.Adam(wrapper.parameters(), lr=1e-3)
...
class TestPaddingMask:
    ...
    def test_mask_shape_tiling(self):
...
class TestAllFusionsEndToEnd:
    ...
    def test_fusion_forward_backward(self, fusion_name: str):
        waveform, sr = load_real_audio(n_samples=2)
        transform = MultibandTransform(sample_rate=sr, return_scores=True)
        bands, scores = transform(waveform)
        B = bands.shape[1]
        ...
        backbone = SmallConvBackbone(embed_dim=EMBED_DIM)
        head = None if fusion_name == "moe" else LinearHead(EMBED_DIM, NUM_CLASSES)
        wrapper = MultibandWrapper(backbone=backbone, fusion=fusion_name,
                                   head=head, embed_dim=EMBED_DIM, **extra)
        optimizer = torch.optim.Adam(wrapper.parameters(), lr=1e-3)
        ...
        assert logits.shape == (2, NUM_CLASSES), \
            f"{fusion_name}: expected (2, {NUM_CLASSES}), got {logits.shape}"

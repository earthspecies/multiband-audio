...
class TestMeanPoolFusion:
    ...
    def test_weights(self, embeddings):
        f = MeanPoolFusion()
        f(embeddings)
...
class TestGatedPoolFusion:
    ...
    def test_weights_sum_to_one(self, embeddings):
        f = GatedPoolFusion(embed_dim=D)
        f(embeddings)
        w = f.get_band_weights()
...
class TestHybridFusion:
    def test_with_scores(self, embeddings, scores):
        f = HybridFusion(embed_dim=D, num_handcrafted=2)
        out = f(embeddings, scores)
        assert out.shape == (N, D)

    def test_without_scores(self, embeddings):
        f = HybridFusion(embed_dim=D, num_handcrafted=2)
...
class TestCrossAttentionFusion:
    ...
    def test_multi_query(self, embeddings):
...
class TestRegistry:
    def test_paper_keys_registered(self):
